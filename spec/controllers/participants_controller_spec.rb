...
describe ParticipantsController do
  ...
  def mock_participant(stubs={})
    @mock_participant ||= mock_model(Participant, stubs)
  end

  describe "GET index" do
    it "assigns all participants as @participants" do
      Participant.stub!(:find).with(:all).and_return([mock_participant])
      get :index
      assigns[:participants].should == [mock_participant]
    end
  end

  describe "GET show" do
    it "assigns the requested participant as @participant" do
      Participant.stub!(:find).with("37").and_return(mock_participant)
      get :show, :id => "37"
      assigns[:participant].should equal(mock_participant)
    end
  end
  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "assigns a newly created participant as @participant" do
        Participant.stub!(:new).with({'these' => 'params'}).and_return(mock_participant(:save => true))
        post :create, :participant => {:these => 'params'}
        assigns[:participant].should equal(mock_participant)
      end

      it "redirects to the created participant" do
        Participant.stub!(:new).and_return(mock_participant(:save => true))
    ...
    describe "with invalid params" do
      it "assigns a newly created but unsaved participant as @participant" do
        Participant.stub!(:new).with({'these' => 'params'}).and_return(mock_participant(:save => false))
        post :create, :participant => {:these => 'params'}
        assigns[:participant].should equal(mock_participant)
      end

      it "re-renders the 'new' template" do
        Participant.stub!(:new).and_return(mock_participant(:save => false))
        post :create, :participant => {}
  ...
  describe "PUT update" do
    ...
    describe "with valid params" do
      it "updates the requested participant" do
        Participant.should_receive(:find).with("37").and_return(mock_participant)
        mock_participant.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, :id => "37", :participant => {:these => 'params'}
      end

      it "assigns the requested participant as @participant" do
        Participant.stub!(:find).and_return(mock_participant(:update_attributes => true))
        put :update, :id => "1"
        assigns[:participant].should equal(mock_participant)
      end
    ...
    describe "with invalid params" do
      ...
      it "assigns the participant as @participant" do
        Participant.stub!(:find).and_return(mock_participant(:update_attributes => false))
        put :update, :id => "1"
        assigns[:participant].should equal(mock_participant)
      end
  ...
  describe "DELETE destroy" do
    ...
    it "redirects to the participants list" do
      Participant.stub!(:find).and_return(mock_participant(:destroy => true))
      delete :destroy, :id => "1"
