    ...
    def build_percentages_collection c, duration
      ...
      c.each do |item|
        if status == item[:status]
          @percentage_collection.last[:width] = format(width+item[:percentage])
        else
          @percentage_collection << {:status => item[:status], :left => format(sum), :width => format(item[:percentage])}
        end
        width = item[:percentage]
        status = item[:status]
        sum += width
      end
      @percentage_collection.last[:width] = format(100-@percentage_collection.last[:left].to_f)
    end
  ...
  class << self
    ...
    def details_for *ids
      ...
      Report.benchmark("DETAILS") do
        Details.new(@data, conditions[:from].to_i, conditions[:to].to_i)
      end
    end
